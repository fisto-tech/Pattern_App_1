...
const MAX_UPLOAD_FILE_SIZE = 2 * 1024 * 1024; // 2 MB
...
function getFileSizeError(file, maxSize) {
  return `File Size:\nPlease upload a file smaller than ${formatFileSize(maxSize)}. 
  Your current file size is ${formatFileSize(file.size)}.`;
}

function getImageDimensionError(req, actual, label) {
  return `Image Size:\nPlease upload a ${label} image with a size of ${req.w} × ${req.h} px. 
  Your current image size is ${actual.w} × ${actual.h} px.`;
}
...
async function validatePatternDimensions(file, type, shape) {
  ...
  if (w < req.w || h < req.h) {
    return {
      valid: false,
      error: getImageDimensionError(req, { w, h }, type === "top" ? "Lid" : "Tub"),
    };
  }
  return { valid: true };
}
  ...
  lb.innerHTML = `
    ...
    <div class="lightbox-container">
      ...
      <div id="lightbox-content-single" class="lightbox-content" style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; height: 100%;">
        ...
        <button id="lightbox-download-btn" class="save_btn" style="margin-top: 1vw; padding: 0.7vw 1.2vw; font-size: 0.85vw;">Download Image</button>
      </div>
      ...
      <div id="lightbox-content-dual" style="display: none; width: 95%; height: 85vh; gap: 2vw; align-items: center; justify-content: center; z-index: 10;">
        <div style="flex: 1; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1vw;">
           ...
           <button id="lightbox-download-lid-btn" class="save_btn" style="font-size: 0.85vw; min-width: 10vw; padding: 0.7vw 1.2vw;">Download Lid</button>
        </div>
        <div style="flex: 1; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 1vw;">
           ...
           <button id="lightbox-download-tub-btn" class="save_btn" style="font-size: 0.85vw; min-width: 10vw; padding: 0.7vw 1.2vw;">Download Tub</button>
        </div>
      </div>
      ...
      <div id="lightbox-content-vertical" style="display: none; width: 95%; height: 90vh; flex-direction: column; gap: 1vw; align-items: center; justify-content: flex-start; z-index: 10;">
        <div style="width: 100%; height: 60%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.5vw;">
           ...
           <button id="lightbox-download-lid-ver-btn" class="save_btn" style="font-size: 0.85vw; min-width: 10vw; padding: 0.7vw 1.2vw;">Download Lid</button>
        </div>
        <div style="width: 100%; height: 30%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.5vw;">
           ...
           <button id="lightbox-download-tub-ver-btn" class="save_btn" style="font-size: 0.85vw; min-width: 10vw; padding: 0.7vw 1.2vw;">Download Tub</button>
        </div>
      </div>
...
function getCurrentLightboxUrls() {
  ...
  const mainUrl = view === "lid" ? urlTop : urlTub;
  return { urlTop, urlTub, mainUrl };
}
...
function getFileNameFromUrl(url) {
  try {
    ...
    const name = pathname.substring(pathname.lastIndexOf("/") + 1) || "pattern";
...
async function downloadUrlAsFile(url) {
  if (!url) return;
...
function updateLightboxView() {
  ...
  const downloadLidBtn = document.getElementById("lightbox-download-lid-btn");
  const downloadTubBtn = document.getElementById("lightbox-download-tub-btn");
  const downloadLidVerBtn = document.getElementById("lightbox-download-lid-ver-btn");
  const downloadTubVerBtn = document.getElementById("lightbox-download-tub-ver-btn");
  ...
  if (isSweetBoxTE && urlTop && urlTub) {
    ...
    if (dualBox) {
      ...
      if (downloadLidBtn) {
        downloadLidBtn.style.display = "inline-flex";
        downloadLidBtn.onclick = () => downloadLightboxImage(urlTop);
      }
      if (downloadTubBtn) {
        downloadTubBtn.style.display = "inline-flex";
        downloadTubBtn.onclick = () => downloadLightboxImage(urlTub);
      }
    }
    if (nextBtn) nextBtn.style.display = "none";
    if (downloadBtn) downloadBtn.style.display = "none";
    if (downloadLidVerBtn) downloadLidVerBtn.style.display = "none";
    if (downloadTubVerBtn) downloadTubVerBtn.style.display = "none";
    ...
    if (verticalBox) {
      ...
      if (downloadLidVerBtn) {
        downloadLidVerBtn.style.display = "inline-flex";
        downloadLidVerBtn.onclick = () => downloadLightboxImage(urlTop);
      }
      if (downloadTubVerBtn) {
        downloadTubVerBtn.style.display = "inline-flex";
        downloadTubVerBtn.onclick = () => downloadLightboxImage(urlTub);
      }
    }
    if (nextBtn) nextBtn.style.display = "none";
    if (downloadBtn) downloadBtn.style.display = "none";
    if (downloadLidBtn) downloadLidBtn.style.display = "none";
    if (downloadTubBtn) downloadTubBtn.style.display = "none";
  } else {
    if (dualBox) dualBox.style.display = "none";
    if (verticalBox) verticalBox.style.display = "none";
    if (singleBox) {
      ...
      if (downloadBtn) {
        ...
        downloadBtn.textContent = `Download ${view === "lid" ? "Lid" : "Tub"} Pattern`;
        downloadBtn.onclick = () => downloadLightboxImage(fullUrl);
      }
    }
    if (nextBtn) {
...
async function handleModalSubmit() {
  ...
  if (pendingFiles.primary) {
    ...
    if (pendingFiles.primary.size > MAX_UPLOAD_FILE_SIZE) {
      sizeErrors.push(getFileSizeError(pendingFiles.primary, MAX_UPLOAD_FILE_SIZE));
    }
  }
  if (pendingFiles.top) {
    const v = await validatePatternDimensions(pendingFiles.top, "top", shape);
    if (!v.valid) sizeErrors.push(v.error);
    if (pendingFiles.top.size > MAX_UPLOAD_FILE_SIZE) {
      sizeErrors.push(getFileSizeError(pendingFiles.top, MAX_UPLOAD_FILE_SIZE));
    }
  }
...
function renderPatternGrid(patterns, gridContainer) {
  ...
  if (selectedShape !== "") {
    ...
    infoEl.innerHTML = dimStr
      ? `<span>Required Dimensions for ${selectedShape}:</span> <strong>${dimStr}</strong> <strong>px</strong> `
      : "";
...
async function uploadPatternHandler() {
  ...
  if (file.size > MAX_UPLOAD_FILE_SIZE)
    return alert(getFileSizeError(file, MAX_UPLOAD_FILE_SIZE));
