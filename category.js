...
function renderCategories(categories, errorMessage = "") {
  ...
  const shapePriority = {
    ...
    "Rectangle": 2,
    "Sweet Box": 3,
    "Sweet Box Tamper Evident": 4,
  };

  const sortedCategories = categories
    .map((cat, originalIndex) => ({ ...cat, _originalIndex: originalIndex }))
    .sort((a, b) => {
      const typeA = String(a.shape_type || "").trim();
      const typeB = String(b.shape_type || "").trim();
      const orderA = shapePriority[typeA] ?? Number.MAX_SAFE_INTEGER;
      const orderB = shapePriority[typeB] ?? Number.MAX_SAFE_INTEGER;
