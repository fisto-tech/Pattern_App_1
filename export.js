window.addEventListener("DOMContentLoaded", () => {
  ...
  // Create loading spinner overlay for model transitions
  function createModelLoadingOverlay() {
    const overlay = document.createElement("div");
    overlay.id = "modelLoadingOverlay";
    overlay.className = "model-loading-overlay hidden";
    overlay.innerHTML = `
      <div class="model-loading-spinner"></div>
      <div class="model-loading-text">Loading...</div>
    `;
    return overlay;
  }

  // Add loading overlay to model viewer (not the entire model_view container)
  const modelLoadingOverlay = createModelLoadingOverlay();
  if (modelViewer) {
    modelViewer.appendChild(modelLoadingOverlay);
  }

  let spinnerTimeout = null;
  const MIN_SPINNER_SHOW_TIME = 1000; // Minimum time spinner displays (ms) - 1 second for plain container to sync
  ...
  lidTransparent.addEventListener("change", () => {
    ...
    if (lidTransparent.checked) {
      ...
      if (isPlainContainerEnabled) {
        // Force white opaque when unchecking transparency in Plain Container mode
        topColor.value = "#f7f7f7";
        lidColorManualSet = true;
      } else {
        // Normal behavior: restore previous color
  ...
  if (zoomInput) {
    ...
    zoomInput.addEventListener("keydown", (e) => {
      ...
      // Cancel on Escape - restore current zoom display
      if (e.key === "Escape") {
    ...
    // Apply zoom when input loses focus
    ...
    // Select all text when user clicks the input for easy editing
    zoomInput.addEventListener("focus", () => {
      zoomInput.select();
    });

    // Prevent scroll from changing zoom input value accidentally
  ...
  // Renumber all remaining cards sequentially
  function renumberAllCards() {
    const allCards = renderedImages.querySelectorAll(".rendered-card");
    allCards.forEach((card, index) => {
      const newNumber = index + 1;
      card.dataset.id = newNumber;

      // Update the option-theme text
      const optionTheme = card.querySelector(".option-theme");
      if (optionTheme) {
        const title = card.dataset.title || "Untitled";
        optionTheme.textContent = `Option - ${newNumber}   Theme - ${title}`;
      }
    });

    // Reset cardCounter to the next available number
    cardCounter = allCards.length + 1;
  }
    ...
    selectedModel.angles.forEach((angle, i) => {
      ...
      angleCard.addEventListener("click", () => {
        ...
        if (!removeDefaultDesign || !removeDefaultDesign.checked) {
          ...
          // Force materials back to OPAQUE before new angle loads
          if (modelViewer && modelViewer.model) {
            updatePartTransparency(modelViewer, "lid", false, null);
            updatePartTransparency(modelViewer, "tub", false, null);
          }
  ...
  function restoreOriginalModel() {
    // Reset all manual override flags so syncViewerState reads GLB defaults
    lidColorManualSet = false;
    tubColorManualSet = false;
    lidTransparencyManualSet = false;
    tubTransparencyManualSet = false;
    ...
    // Clear any applied textures
    currentTubTextureDataURL = null;
    currentLidTextureDataURL = null;

    // Clear the original materials cache so fresh defaults are read from the new load
    originalMaterialsCache.clear();

    // Force the model to reload by clearing lastLoadedSrc
    lastLoadedSrc = null;

    // Reset color pickers to white (they will be overwritten by syncViewerState on load)
    topColor.value = "#ffffff";
    tubColor.value = "#ffffff";

    // Reset background
    bgColor.value = "#c7c7c7";
    modelbg.style.background = "#c7c7c7";
    updateBackgroundLuminance("#c7c7c7");

    // If "Remove Default Design" is still checked, keep plain container state
    // Otherwise reload the current model to get original IML textures back
    if (removeDefaultDesign && removeDefaultDesign.checked) {
      // Plain container stays active, just re-sync
      syncViewerState(modelViewer);
    } else {
      // Force a full model reload to restore original IML textures
      // by re-setting the src attribute (triggers the "load" event)
      const currentSrc = mainModelViewer.getAttribute("src");
      mainModelViewer.removeAttribute("src");

      // Use requestAnimationFrame to ensure the removal is processed before re-setting
      requestAnimationFrame(() => {
        mainModelViewer.setAttribute("src", currentSrc);
        // syncViewerState will be called automatically by the "load" event listener
      });
    }
  }
  ...
  exportBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      await Promise.all([
        ...
        new Promise((res) => {
          customLogo.onload = res;
          customLogo.onerror = () => { console.warn("Custom logo failed to load"); res(); };
        }),
      ]);
      ...
      for (let i = 0; i < sortedModels.length; i++) {
        ...
        const tubTextureDataURL = card.dataset.textureDataUrl || null;
        ...
        if (needsHighResCapture) {
          ...
          try {
            ...
            pdfModelViewer = await createPDFModelViewer(
              modelSrcForCard,
              tubTextureDataURL,
              topMaterialColor,
              3840,
            ...
            console.error("Model capture failed for:", modelTitle, error);
            modelImageData = snapshotDataURL;
          } finally {
            // Always clean up the PDF model viewer
        ...
        // Final fallback: use snapshot if high-res capture wasn't attempted or failed
        if (!modelImageData) {
          modelImageData = snapshotDataURL;
        }
        ...
        if (modelImageData) {
          try {
            ...
            await new Promise((res, rej) => {
              img.onload = res;
              img.onerror = () => {
                console.warn("Model image failed to load for PDF page");
                res(); // Resolve instead of reject to prevent PDF crash
              };
            ...
            if (img.width > 0 && img.height > 0) {
              ...
              if (imageAspectRatio > availableAspectRatio) {
                finalWidth = availableWidth * 0.75;
                finalHeight = finalWidth / imageAspectRatio;
              } else {
                finalHeight = availableHeight * 0.75;
              ...
              pdf.text("Image could not be rendered", contentCenterX, contentCenterY, {
                align: "center",
              });
            }
          } catch (imgError) {
            console.error("Failed to add image to PDF page:", imgError);
            pdf.setFont("helvetica", "italic");
            pdf.setFontSize(14);
            pdf.setTextColor(150, 150, 150);
            pdf.text("Image could not be rendered", contentCenterX, contentCenterY, {
          ...
          console.warn(`No image data available for card: ${modelTitle}`);
          pdf.setFont("helvetica", "italic");
          pdf.setFontSize(14);
          pdf.setTextColor(150, 150, 150);
          pdf.text("Plain Container - No texture applied", contentCenterX, contentCenterY, {
            align: "center",
          });
        }
  ...
  downloadModelBtn.addEventListener("click", async () => {
    ...
    // Check if there are rendered cards and warn the user
    // const allRenderedCards = renderedImages.querySelectorAll(".rendered-card");
    // if (allRenderedCards.length > 0) {
    //   // Show confirmation dialog and wait for user response
    //   const userConfirmed = await new Promise((resolve) => {
    //     showCustomModal({
    //       title: "Warning: Rendered Cards Present",
    //       message:
    //         "There are render cards present in the Rendered Models section. If you export as " +
    //         format.toUpperCase() +
    //         ", whatever edited in the render cards would be lost. Do you want to continue?",
    //       type: "confirm",
    ...
    try {
      ...
      // Do NOT reload the page - preserve rendered cards
    } catch (error) {
